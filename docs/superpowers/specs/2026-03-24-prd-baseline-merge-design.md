...
首次使用新版 Project Scanner 扫描时：
1. 检查 `reverse-prd-doc-id.yml` 是否存在，若存在则读取旧 doc_id 复用（更新而非新建飞书文档）
2. 扫描完成后，删除 GitHub 上的 `docs/prd/project-prd-draft.md` 和 `docs/development-status.md`
3. 将 `reverse-prd-doc-id.yml` 重命名为 `baseline-doc-id.yml`
4. 飞书文档标题更新为「[项目名] 项目基线文档」

### 数据源优先级（source of truth）

`feature-tracking.json` 是程序化的权威数据源。基线 Markdown 始终从此 JSON 重新生成功能清单章节。当用户通过飞书编辑基线后手动触发"整理基线"时，LLM 读取飞书文档内容，反向更新 `feature-tracking.json`，再重新生成 Markdown。
...
def read_file_head(self, repo: str, path: str, max_lines: int = 100) -> str:
    """读取文件前 N 行。内部调用 read_file 获取全文后本地截断。"""

def get_pr_commits(self, repo: str, pr_number: int) -> list[dict]:
    """获取 PR 关联的所有 commit messages。返回 [{sha, message, author}]。"""
```

**`read_file_head` 实现说明**：GitHub API 无法只取文件前 N 行，实际下载全文后本地截断。通过 `get_repo_tree` 返回的 `size` 字段预过滤，跳过大于 50KB 的文件。

**`get_pr_commits` 用途**：`prd_baseline` 模块在 PR 合并时需要获取该 PR 的 commit messages 来做功能匹配。GitHub webhook 的 `pull_request` payload 不包含 commit 列表，需要额外 API 调用。
...
commit messages 可能有数百条，需要分批处理：

```
1. 将 commits 按每 80 条分批
2. 每批调用 LLM：将这些 commit 按功能分组归纳
   输出 JSON：[{"feature": "名称", "commits": ["sha1", "sha2"], "description": "一句话"}]
3. 合并各批结果，去重（相同 feature 名称合并）
4. 最终输出统一的功能列表
```

注：原有的 `commit_classifier.py`（按 feature/bugfix/refactor 分类）仍被 Daily Report 使用，不废弃。Project Scanner 的功能推导改用独立的聚类 prompt，两者互不影响。
...
生成基线草稿 → 飞书 Wiki + GitHub
  ↓
发飞书确认卡片：[✅ 确认] [📝 需要调整]
  确认 → 写入独立文件 baseline-confirmed.yml: {confirmed: true, date: "..."}
  调整 → 提示在飞书编辑后再确认

注：确认标记存于独立的 `baseline-confirmed.yml`，不写入 `latest-scan.json`。避免扫描失败后 `latest-scan.json` 被覆盖导致误判。
```
...
### LARK_CARD_ACTION 路由设计

prd_baseline 和 project_scanner 都需要处理飞书卡片按钮回调。通过卡片 value 中的 `action` 字段区分：

| action 值 | 处理模块 | 含义 |
|-----------|---------|------|
| `confirm_baseline_merge` | prd_baseline | 确认将新 PRD 合并到基线 |
| `skip_baseline_merge` | prd_baseline | 暂不合并 |
| `confirm_feature_status` | prd_baseline | 确认功能状态变更 |
| `reject_feature_status` | prd_baseline | 否认功能关联 |
| `confirm_cold_start` | project_scanner | 确认冷启动基线 |
| `adjust_cold_start` | project_scanner | 需要调整冷启动基线 |
| `confirm_scan_gap` | prd_baseline | 确认扫描发现的遗漏功能 |

各模块订阅 `LARK_CARD_ACTION`，在 handler 中检查 `event.payload["action"]["value"]["action"]` 是否属于自己的 action 集合，不属于则 return 忽略。

### INTERNAL_PRD_FINALIZED payload 扩展

需要修改 PRD Generator，在派发事件时增加 `github_path` 字段：

```python
# prd_generator/handler.py _generate_prd() 中
await self.bus.dispatch(Event(
    ...
    payload={
        ...
        "github_path": github_path,  # 新增
    },
    member=None,
))
```

prd_baseline 从 payload 获取 topic 和 github_path。summary 通过读取 PRD 文档首段提取。
...
INTERNAL_PRD_FINALIZED
  ↓ prd_baseline 监听
  从 payload 获取 topic, github_path
  读取 PRD 文档首段作为 summary
发飞书确认卡片：
  "是否将「用户反馈系统」合并到项目基线？"
  [✅ 合并] value: {"action": "confirm_baseline_merge", "topic": "...", "prd_path": "..."}
  [❌ 暂不合并] value: {"action": "skip_baseline_merge", "topic": "..."}
  ↓ 确认（LARK_CARD_ACTION, action=confirm_baseline_merge）
结构化追加到基线"⬚ 待开发"章节：
  - ⬚ **用户反馈系统** — 反馈收集与分析 → [详细 PRD](prd-用户反馈系统.md)
同步飞书 Wiki + GitHub
...
读取基线中 🔄 进行中 + ⬚ 待开发的功能列表
调用 github.get_pr_commits(repo, pr_number) 获取 commit messages
LLM 匹配 → 输出 JSON 数组（支持一个 PR 涉及多个功能）：
  [
    {
...
基线文档是面向人的 Markdown，此 JSON 是面向程序的权威数据源（见 Section 1 数据源优先级）。

### 存储结构汇总

```
.grove/memory/project-scan/
  ├── baseline-doc-id.yml       # 飞书文档 ID
  ├── baseline-confirmed.yml    # 冷启动确认标记（独立文件）
  ├── feature-tracking.json     # 功能状态索引（权威数据源）
  ├── latest-scan.json          # 扫描元数据
...
grove/modules/communication/handler.py           # +1 路由 + MODULE_DISPLAY 新增 "prd_baseline": "PRD 基线"

grove/integrations/github/client.py              # +read_file_head, +get_pr_commits
grove/integrations/lark/cards.py                 # +2 卡片

grove/modules/prd_generator/handler.py           # INTERNAL_PRD_FINALIZED payload 增加 github_path
...
| PRD Generator payload 扩展 | 0 | 1 | 低 |
| 测试 | 3 | 1 | — |
| **合计** | **8** | **14** | — |
