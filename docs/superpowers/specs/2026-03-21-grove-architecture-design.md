...
| 内部 | `internal.new_requirement` | 交互沟通模块识别出新需求意图 | PRD 生成 |
...
- **Handler 异常捕获** — 事件总线捕获每个 handler 的异常，记录到 `.grove/logs/events.log`，不影响其他 handler 执行
- **自动重试** — 外部 API 调用（GitHub / 飞书 / Claude）失败时，最多重试 3 次，指数退避（1s → 2s → 4s）
- **失败事件日志** — 重试耗尽后，将事件写入 `.grove/logs/failed-events.jsonl`，包含事件内容、错误信息、时间戳，支持手动排查和重放
- **LLM 超时** — Claude API 调用设置 60s 超时，超时视为失败进入重试
- **平台不可用** — GitHub / 飞书 API 持续不可用时，事件积压在内存队列中（上限 1000 条），恢复后自动处理；超出上限的事件丢弃并记录日志
...
│   ├── github_webhook.py            # FastAPI 路由：POST /webhook/github（验证 X-Hub-Signature-256）
│   ├── lark_websocket.py            # 飞书 WebSocket 长连接客户端（SDK 内置签名验证）
...
| **成员管理** | team.yml 加载 + 成员当前任务/负载缓存 + 工作量查询 | `internal.task_assigned`, `cron.daily_report` | 成员信息与负载查询（供其他模块调用） |
...
**Webhook 安全：** 入站 Webhook 必须验证 `X-Hub-Signature-256` 请求头，使用 Webhook Secret 进行 HMAC-SHA256 签名校验，拒绝无效请求
...
**并发控制：** LLM Client 内置信号量（默认并发上限 3），防止多个事件同时触发时超出 Anthropic API 速率限制。每次调用记录 token 用量到 `.grove/logs/llm-calls.log`，支持月度成本追踪。
...
│   └── conversations/          # 进行中的多轮对话状态（如 PRD 引导提问）
│       └── conv-abc123.json    # 进程重启后可恢复
│
...
# 文档同步
doc_sync:
  auto_update_level: "moderate"         # conservative: 仅自动更新小改，中/大改都需确认
                                        # moderate: 小改自动，中改草稿确认，大改团队讨论（默认）
                                        # aggressive: 小/中改都自动，仅大改需确认
...
team:
  - github: allen
    lark_id: "ou_xxxxxxxx0"
    name: Allen
    role: fullstack
    skills: [python, react, devops]
    authority: owner              # 项目负责人，可修改 Grove 配置和覆盖决策
...
- **文件格式版本：** config.yml 和 team.yml 均包含 `version` 字段，未来格式变更时提供迁移脚本

### 6.5 健康检查

`GET /health` 端点返回以下状态，用于 Docker `HEALTHCHECK` 和运维监控：

```json
{
  "status": "healthy",
  "fastapi": true,
  "lark_ws_connected": true,
  "scheduler_running": true,
  "last_event_processed": "2026-03-21T10:30:00+08:00"
}
```

### 6.6 飞书文档变更事件

`lark.doc_updated` 事件通过飞书事件订阅的 HTTP 回调接收（`POST /webhook/lark`），不走 WebSocket 通道。飞书文档变更属于应用事件订阅，需在飞书开放平台配置回调 URL。入站请求使用飞书 SDK 内置的签名验证。
